...
export default function Dashboard() {
  const { user, hasRole, selectedInstitution } = useAuth();
  const isSuperAdmin = hasRole('superadmin');

  // A superadmin with no institution selected would send requests with no
  // x-company-id header, which resolveCompanyScope rejects with a 400.
  // Skip loading entirely and show a prompt instead.
  const skipLoad = isSuperAdmin && !selectedInstitution;
      ...
      {skipLoad ? (
        <div className="card">
          <p style={{ color: 'var(--muted)' }}>
            Select an institution from the <strong>Institutions</strong> list to view its dashboard.
          </p>
        </div>
      ) : (
        <>
